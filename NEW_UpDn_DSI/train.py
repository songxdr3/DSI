...
def compute_sample_difficulty(pred):
    """
    计算样本的难度（基于预测概率的熵）
    pred: 模型的预测输出 (batch_size, num_classes)
    """
    probs = torch.softmax(pred, dim=1)  # 计算 softmax 概率
...
# 修改map_confidence_to_shuffle_ratio函数
...
def adjusted_labels(labels, head_classes, tail_classes, mid_classes, delta_head=0.13, delta_tail=0.00, delta_mid=0.00):
    ...
    # 初始化 delta，默认使用中部类别的 delta
    #delta = torch.full((batch_size,), delta_mid, device=labels.device)
    ...
    # **第二步**: 先赋值 `delta / num_classes` 到 `adjusted_labels[head_mask]`
    ...
    # **第三步**: 将 `adjusted_labels[(head_mask) & (labels != 0)]` 赋值 `1 - delta + delta / num_classes`
    # 找到所有非零的索引
...
def train(model, qmodel, train_loader, eval_loader, qid2type, args):
    ...
    head_threshold = int(0.4 * num_classes)  # 头部类别数量
    mid_threshold = int(0.7 * num_classes)  # 头部 + 中部数量
