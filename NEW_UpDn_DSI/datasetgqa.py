...
def _load_dataset(dataroot, name, img_id2val, dataset):
    ...
    """
    if dataset == 'cpv2':
        answer_path = os.path.join(dataroot, 'vqa/cp-cache', '%s_target.pkl' % name)
        name = "train" if name == "train" else "test"
        question_path = os.path.join(dataroot, 'vqa/vqacp_v2_%s_questions.json' % name)
        with open(question_path) as f:
            questions = json.load(f)
    elif dataset == 'cpv1':
        answer_path = os.path.join(dataroot, 'vqa/cp-v1-cache', '%s_target.pkl' % name)
        name = "train" if name == "train" else "test"
        question_path = os.path.join(dataroot, 'vqa', 'vqacp_v1_%s_questions.json' % name)
        with open(question_path) as f:
            questions = json.load(f)
    elif dataset == 'v2':
        answer_path = os.path.join(dataroot, 'vqa/cache', '%s_target.pkl' % name)
...
class VQAFeatureDataset(Dataset):
    def __init__(self, name, dictionary, dataroot='../data', dataset='cpv2',
        ...
        if dataset == 'cpv2':
            ans2label_path = os.path.join(dataroot, 'vqa/cp-cache', 'trainval_ans2label.pkl')
            label2ans_path = os.path.join(dataroot, 'vqa/cp-cache', 'trainval_label2ans.pkl')
        elif dataset == 'cpv1':
        ...
        if use_hdf5:
            ...
            self.spatial = {}
            self.image_id2ix = {}
            h5_path_train = os.path.join('/opt/data/private/sxd/data/gqa/vg_gqa_imgfeat/', 'output.h5')
            self.hf_train = h5py.File(h5_path_train, 'r')
            self.features = self.hf_train.get('features')

            self.image_id2ix = cPickle.load(
                open(os.path.join('/opt/data/private/sxd/data/gqa/vg_gqa_imgfeat/', f'imgid2img.pkl'), 'rb'), encoding="latin1")
        ...
        if cache_image_features:
            ...
            for entry in tqdm(self.entries, ncols=100, desc="caching-features"):
                ...
                if img_id not in image_to_fe:
                    if use_hdf5:
                        fe = np.array(self.features[self.image_id2ix[img_id]])
    ...
    def __getitem__(self, index):
        ...
        if self.image_to_fe is not None:
            ...
            features = np.array(self.features[entry['img_idx']])
